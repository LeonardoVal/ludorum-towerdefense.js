(function() {
	...
	var addUnitInfo = function(level, marios, ropes, firewizzrobes, darknuts, speedies, armos) {
		level += 1;
		...
		divM.innerHTML = [
			...
			'<div class=title>', marios, '</div><div class=info>'
		].join('');
		...
		divR.innerHTML = [
			...
			'<div class=title>', ropes, '</div><div class=info>'
		].join('');
		...
		divF.innerHTML = [
			...
			'<div class=title>', firewizzrobes, '</div><div class=info>'
		].join('');
		...
		divD.innerHTML = [
			...
			'<div class=title>', darknuts, '</div><div class=info>'
		].join('');
		...
		divS.innerHTML = [
			...
			'<div class=title>', speedies, '</div><div class=info>'
		].join('');
		...
		divA.innerHTML = [
			...
			'<div class=title>', armos, '</div><div class=info>'
		].join('');
	...
	var completed = function(e) {
		var scenario = 1;
		for (var j = 0; j < logic.waves.waves.length; j++) {
			var wave0 = logic.waves.waves[j].units;
			...
			addUnitInfo(j, cantmarios, cantropes, cantfirewizzrobes, cantdarknuts, cantspeedies, cantarmos);
		}
		addTowers();
		addHandlers();
		...
		if (scenario == 2) {
			...
			if (scenario == 3) {
				...
				if (scenario == 4) {
					...
					console.log("Scenario incorrecto");
					path5 = [new Point(1,0), new Point(1,1), new Point(1,2), new Point(1,3), new Point(1,4), new Point(2,4), new Point(3,4), new Point(3,5), new Point(3,6), new Point(4,6), new Point(5,6), new Point(5,5), new Point(5,4), new Point(5,3), new Point(5,2), new Point(5,1), new Point(5,0)];
					turrets = [new Point(2,3), new Point(4,5), new Point(4,3), new Point(2,1), new Point(2,5)];
				}
			}
		}
...
function EndGame()
{
	...
	firebase.database().ref('matches/' + 1 + '/' + window.user.uid).push({
